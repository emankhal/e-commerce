...
export default function ProductDetails() {
  ...
  async function getCategory() {
    ...
    let newData = data.data.filter((product) => product?.category?.name === category);
  ...
  return (
    <div>
      {product ? <div className="items-center grid grid-cols-[minmax(0,1fr)_minmax(0,2fr)] gap-5">
        ...
        <div className="desc ">
          <div className="description p-3">
            ...
            <h5 className='font-bold my-2'>{product?.title}</h5>
      ...
      </div> : <div role="status " className='absolute top-[50%] left-[50%] -translate-x-1/2 -translate-y-1/2'>
